...
- Install JavaScript packages with `bun add` or `bun add -d`. Do not manually add or change package versions in `package.json`.
- Install PHP packages with `composer require` or `composer require --dev`. Do not manually add or change package versions in `composer.json`.
